...
import com.bjvca.commonutils.{ConfUtils, DataSourceUtil, QueryCallback, SqlProxy}
...
/**
 * 使用德鲁伊连接池，对MySQL库进行操作
 * 在MySQL当中维护一个task表当作任务队列
 */
...
object Cleaned1 extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    sqlProxy.executeQuery(client,"",Array(),new QueryCallback {
    ...
    while (true) {
      ...
      val value: RDD[String] = spark.sql("select * from ccc")
        .toJSON
        .rdd
      ...
      // TODO 修改task表status为1
      value.foreach(s => {
        //      val str = s.asInstanceOf[JSONObject].getString("video_id")
        //      println(s)
        ...
        val client = DataSourceUtil.getConnection
        try {
          sqlProxy.executeUpdate(client, "replace into `task` (video_id,status) values(?,1)",
            Array(vid))
        }
        catch {
